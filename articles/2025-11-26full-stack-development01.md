...
- Server Component: サーバーで実行され、完成したHTMLをまとめて配る「先生役」。データ取得が得意で、クラス全員に同じ資料を配るイメージ。
- Client Component: ブラウザで動く「生徒の手元のノート」。ボタンのクリックや入力にすぐ反応する。動的UIや状態管理に向く。
- 使い分けのコツ: 読むだけのページやデータ取得はServer、ユーザー操作がある部分だけClientにする、と覚えるとラク。
